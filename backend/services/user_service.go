...
func GetUserByID(id uuid.UUID) (models.User, error) {
	...
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			log.Printf("INFO: Usuário com ID %s não encontrado: %v", id, result.Error)
		} else {
...
func UpdateUser(user *models.User, id uuid.UUID) error {
	// A lógica de hashing de senha foi removida pois o UpdateUserHandler não lida com atualização de senha.
	// A atualização de senha deve ser tratada por uma função dedicada e um endpoint específico.
